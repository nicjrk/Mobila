...
export function createCutlistPdfHtml(cutlist: CncCutlist, projectName = "Project"): string {
    ...
    .map((sheet) => {
      ...
      const parts = sheet.parts
        .map((item) => {
          ...
          const label = `${item.cabinet} · ${item.label} · ${width.toFixed(1)} × ${height.toFixed(1)} × ${item.thickness.toFixed(1)} mm${operationCount ? ` · ${operationCount} op.` : ""}`;
  ...
  cutlist.parts.forEach((item) => {
    const edgeBand = item.edgeBand
      ...
      : "";
    const key = `${item.cabinet}|${item.label}|${item.kind ?? ""}|${item.width}|${item.height}|${item.thickness}|${edgeBand}|${item.operations?.length ?? 0}`;
    ...
    if (current) current.qty += 1;
      ...
      grouped.set(key, {
        ...
        qty: 1,
        operations: item.operations?.length ?? 0,
        ...(edgeBand ? { edgeBand } : {}),
  ...
  const rows = [...grouped.values()]
    .map(
      (item) =>
        `<tr><td>${esc(item.label)}${item.note ? `<small>${esc(item.note)}</small>` : ""}</td><td>${esc(item.kind ?? "")}</td><td>${item.width.toFixed(1)}</td><td>${item.height.toFixed(1)}</td><td>${item.thickness.toFixed(1)}</td><td>${esc(item.edgeBand ?? "")}</td><td>${item.operations}</td><td>${item.qty}</td></tr>`,
    )
    .join("");
